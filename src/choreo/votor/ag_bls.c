...
verify_affine_pairs( blst_p1_affine const * a,
  ...
  fd_bls_g1_t pair_p[ AG_BLS_VERIFY_MAX+1UL ];
  fd_bls_g2_t pair_q[ AG_BLS_VERIFY_MAX+1UL ];
  for( ulong i=0UL; i<cnt; i++ ) {
    g1_to_fd_bls( pair_p+i, a+i );
    g2_to_fd_bls( pair_q+i, b+i );
  }
  g1_to_fd_bls( pair_p+cnt, &BLS12_381_NEG_G1 );
  g2_to_fd_bls( pair_q+cnt, signature );

  fd_bls_g2_prepared_t const * prepared = NULL;
  if( FD_LIKELY( prepared_candidate!=ULONG_MAX ) ) {
    ...
    if( FD_UNLIKELY( !prepared_entry->valid ) ) {
      ...
      if( FD_UNLIKELY( fd_bls_g2_prepare( &prepared_entry->point, pair_q ) ) ) return 0;
  ...
  if( FD_LIKELY( cnt==1UL && prepared ) )
    result = fd_bls_pairing_finalverify_prepared_checked(
        pair_p, prepared, pair_p+1, pair_q+1 );
  else
    result = fd_bls_pairing_finalverify_checked( pair_p, pair_q, cnt+1UL, 1UL<<cnt );
  return result==1;
}
