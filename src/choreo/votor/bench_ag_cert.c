...
#if FD_HAS_AVX512
static inline void
bench_g1_to_fd_bls( fd_bls_g1_t *        out,
...
static inline void
bench_g2_to_fd_bls( fd_bls_g2_t *        out,
...
bench_breakdown( ag_cert_t const *       cert,
  ...
  blst_p1_affine pair_p_blst[2];
  fd_memcpy( pair_p_blst, &epoch->pubkey_cache.total, sizeof(blst_p1_affine) );
  pair_p_blst[1] = BLS12_381_NEG_G1;
  fd_bls_g1_t pair_p[2];
  bench_g1_to_fd_bls( pair_p,   pair_p_blst );
  bench_g1_to_fd_bls( pair_p+1, pair_p_blst+1 );
  blst_p2        hash[1];
  blst_p2_affine pair_q_blst[2];
  fd_bls_g2_t    pair_q[2];
  int sink = 0;
  ...
  for( ulong i=0UL; i<sample_cnt; i++ ) {
    ...
    then = fd_log_wallclock();
    blst_p2_to_affine( pair_q_blst, hash );
    samples[2][i] = fd_log_wallclock()-then;

    then = fd_log_wallclock();
    sink += blst_p2_deserialize( pair_q_blst+1, cert->inner.final.agg_sig.sig )==BLST_SUCCESS;
    samples[3][i] = fd_log_wallclock()-then;

    then = fd_log_wallclock();
    sink += blst_p2_affine_in_g2( pair_q_blst+1 );
    sink += !blst_p2_affine_is_inf( pair_q_blst+1 );
    samples[4][i] = fd_log_wallclock()-then;

    bench_g2_to_fd_bls( pair_q,   pair_q_blst );
    bench_g2_to_fd_bls( pair_q+1, pair_q_blst+1 );
    then = fd_log_wallclock();
    sink += fd_bls_pairing_finalverify( pair_p, pair_q, 2UL );
    samples[5][i] = fd_log_wallclock()-then;

    then = fd_log_wallclock();
    sink += fd_bls_pairing_finalverify_checked( pair_p, pair_q, 2UL, 2UL );
  ...
  static char const * const label[7] = {
    ...
    "signature deserialize", "BLST standalone subgroup", "fd_bls pair product",
    "fd_bls pair + subgroup"
  };
...
original_final_verify( ag_cert_t const *       cert,
  ...
  for( ulong i=0UL; i<epoch->validator_cnt; i++ ) {
    if( signer_set_test( cert->inner.final.agg_sig.bitmask, i ) ) {
      fd_memcpy( gathered+k*AG_BLS_PUB_SZ, epoch->validators[i].bls_key.bytes, AG_BLS_PUB_SZ );
      k++;
    }
  }
