...
static void
fd_sshttp_tls_sign( void *  ctx,
                    uchar   sig[ static 64 ],
                    uchar const payload[ static 130 ] ) {
  (void)ctx; (void)sig; (void)payload;
  FD_LOG_ERR(( "unexpected TLS sign request" ));
}
...
fd_sshttp_cert_verify_cb( void *        ctx,
  ...
  uchar ctx_len = *p++;
  if( FD_UNLIKELY( p + ctx_len > end ) ) return -1;
  p += ctx_len;

  if( FD_UNLIKELY( p + 3 > end ) ) return -1;
  ulong list_len = ( (ulong)p[0] << 16 ) | ( (ulong)p[1] << 8 ) | (ulong)p[2];
  p += 3;
  if( FD_UNLIKELY( p + list_len > end ) ) return -1;
  ...
  while( p < list_end && chain_cnt < FD_X509_CHAIN_MAX ) {
    if( FD_UNLIKELY( p + 3 > list_end ) ) return -1;
    ulong cert_len = ( (ulong)p[0] << 16 ) | ( (ulong)p[1] << 8 ) | (ulong)p[2];
    p += 3;
    if( FD_UNLIKELY( p + cert_len > list_end ) ) return -1;
    ...
    if( FD_UNLIKELY( p + 2 > list_end ) ) return -1;
    ushort ext_len = (ushort)( ( (ushort)p[0] << 8 ) | (ushort)p[1] );
    p += 2;
    if( FD_UNLIKELY( p + ext_len > list_end ) ) return -1;
    p += ext_len;
  }

  if( FD_UNLIKELY( chain_cnt == 0 ) ) return -1;
...
fd_sshttp_tls_init( fd_tls_t * tls, fd_sshttp_t * http ) {
  ...
  tls->sign.ctx = NULL;
  tls->sign.sign_fn = fd_sshttp_tls_sign;
