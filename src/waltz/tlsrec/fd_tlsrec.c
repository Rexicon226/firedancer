...
/* RFC 8446 Section 5.3: per-record nonce = write_iv XOR (0-padded seq) */
...
/* RFC 8446 Section 5.2: AEAD decrypt/encrypt with 5-byte record header as AAD */
...
cb_secrets( void const * hs, void const * rx_secret, void const * tx_secret, uint level ) {
  ...
  /* RFC 8446 Section 7.3: derive traffic keys from secrets */
...
fd_tlsrec_rx( fd_tlsrec_conn_t * conn, fd_tlsrec_slice_t * tcp_rx, fd_tlsrec_slice_t * app_rx ) {
  ...
  /* ChangeCipherSpec so ignore (RFC 8446 Appendix D.4) */
  ...
  if( FD_UNLIKELY( !fd_tlsrec_decrypt( pt, c, c_sz, hdr, conn->read_seq, tag, keys ) ) ) {
    FD_LOG_ERR(( "TLS decrypt failed (seq=%lu sz=%lu)", conn->read_seq, c_sz ));
    return FD_TLSREC_ERR_CRYPTO;
  }
  conn->read_seq++;

  /* Strip padding and content type (RFC 8446 Section 5.4) */
  ulong p_sz = c_sz;
