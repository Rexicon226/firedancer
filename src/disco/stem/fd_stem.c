...
/* Cooperative idle backoff tuning.  Only used when the run loop is told
   the host is CPU oversubscribed (cooperative_idle, see fd_topo.h).  A
   tile that finds no work for STEM_IDLE_SPIN_MAX consecutive iterations
   starts relinquishing its core via clock_nanosleep, ramping the sleep
   from STEM_IDLE_SLEEP_STEP_NS up to STEM_IDLE_SLEEP_MAX_NS, so that
   tiles which do have work can be scheduled instead of every tile
   busy-spinning.  The spin prefix keeps brief idle gaps low latency;
   the cap bounds the idle->busy wakeup latency. */
#ifndef STEM_IDLE_SPIN_MAX
#define STEM_IDLE_SPIN_MAX (128UL)
#endif
#ifndef STEM_IDLE_SLEEP_STEP_NS
#define STEM_IDLE_SLEEP_STEP_NS (1024UL) /* 1 us per idle iteration past the spin prefix */
#endif
#ifndef STEM_IDLE_SLEEP_MAX_NS
#define STEM_IDLE_SLEEP_MAX_NS (128UL*1024UL) /* 128 us */
#endif
...
STEM_(run1)( ulong                        in_cnt,
  ...
  /* Cooperative idle backoff state (see cooperative_idle).  did_work
     records whether the previous iteration made progress; idle_cnt
     counts consecutive iterations without progress. */
  ulong idle_cnt = 0UL;
  int   did_work = 0;
  ...
  for(;;) {
    ...
    /* If the host is oversubscribed, an idle tile relinquishes its core
       instead of busy-spinning so the scheduler can run a tile that has
       work.  Tiles that keep finding work never sleep (idle_cnt stays
       low), and non-oversubscribed deployments skip this entirely. */
    if( FD_UNLIKELY( cooperative_idle ) ) {
      if( FD_LIKELY( did_work ) ) {
        idle_cnt = 0UL;
      } else if( FD_UNLIKELY( ++idle_cnt > STEM_IDLE_SPIN_MAX ) ) {
        ulong sleep_ns = fd_ulong_min( (idle_cnt-STEM_IDLE_SPIN_MAX)*STEM_IDLE_SLEEP_STEP_NS, STEM_IDLE_SLEEP_MAX_NS );
        struct timespec const ts = { .tv_sec=0L, .tv_nsec=(long)sleep_ns };
        clock_nanosleep( CLOCK_REALTIME, 0, &ts, NULL );
        long next = fd_tickcount();
        metric_regime_ticks[0] += (ulong)(next - now);
        now = next;
      }
      did_work = 0;
    }
  ...
  /* Check if we are backpressured.  If so, count any transition into
    ...
    /* A fragment is available on this input: the tile has work this
       iteration, so reset the cooperative idle backoff. */
    did_work = 1;
...
STEM_(run)( fd_topo_t *      topo,
  ...
  /* Network I/O tiles (xdp "net" and "sock") busy-poll kernel queues and
     sockets and drive both RX draining and TX flushing.  The cooperative
     idle backoff would put them to sleep when idle, adding latency to
     packet RX/TX, so they always busy-spin even on an oversubscribed host
     where other tiles cooperatively idle. */
  int cooperative_idle = topo->cooperative_idle;
  if( FD_UNLIKELY( !strcmp( tile->name, "net"    ) || 
                   !strcmp( tile->name, "sock"   ) || 
                   !strcmp( tile->name, "accdb"  ) ||
                   !strcmp( tile->name, "shred"  ) ||
                   !strcmp( tile->name, "execrp" ) ) ) cooperative_idle = 0;
