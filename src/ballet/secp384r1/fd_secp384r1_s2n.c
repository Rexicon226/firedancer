...
/* p \equiv 3 (mod 4), so sqrt(a) = a^((p+1)/4) mod p. */
static inline fd_secp384r1_fp_t *
...
fd_secp384r1_double_scalar_mul_base( fd_secp384r1_point_t *        r,
  ...
  /* No p384_scalarmulbase in s2n-bignum */
  ulong u1G[18];
  p384_montjscalarmul( u1G, (ulong *)u1->limbs, (ulong *)fd_secp384r1_const_g_mont );
  p384_montjscalarmul( r->x->limbs, (ulong *)u2->limbs, (ulong *)a->x->limbs );
  p384_montjadd( r->x->limbs, r->x->limbs, u1G );
}
