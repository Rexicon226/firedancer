...
fd_ed25519_point_sub_with_opts( fd_ed25519_point_t *       r,
  ...
  /* `neg` always comes out in alt form */
  fd_ed25519_point_t neg[1];
  if (b_is_precomputed) {
    // fd_ed25519_point_neg_precomputed( r, b );
    FD_R52X5_QUAD_MOV( r->P, b->P );
    return r;
  } else {
    abort();
    fd_ed25519_point_neg( neg, b );
  }
  return fd_ed25519_point_add_with_opts( r, a, neg, b_Z_is_one, 1, skip_last_mul );
}
